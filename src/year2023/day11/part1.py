...
def expand_space(galaxies, empty_rows, empty_cols):
    new_galaxies = []

    for x, y in galaxies:
        x_diff = sum(1 for col in empty_cols if x > col)
        y_diff = sum(1 for row in empty_rows if y > row)
...
def main():
    ...
    galaxies = expand_space(galaxies, empty_rows, empty_cols)
