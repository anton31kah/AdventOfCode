...
def cached(fun):
    ...
    def wrapper(valves, distances, time_left, open_valves, current_valve):
        key = time_left, open_valves, current_valve
        if key not in cache:
            cache[key] = fun(valves, distances, time_left, open_valves, current_valve)
        return cache[key]
...
@cached
def find_max(valves: list[ValveNode], distances: dict[tuple[str, str], int], time_left: int, open_valves: frozenset[str], current_valve: str):
    if time_left < 0:
        return 0
    ...
    for valve in valves:
        if valve.valve_id not in open_valves and valve.flow > 0 and valve.valve_id != current_valve:
            cost = distances[(valve.valve_id, current_valve)] + 1
            value = find_max(valves, distances, time_left - cost, open_valves | {valve.valve_id}, valve.valve_id)
...
def main():
    ...
    value = find_max(valves, distances, 30, frozenset(), 'AA')
